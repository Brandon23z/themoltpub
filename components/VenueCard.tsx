...
export default function VenueCard({
  ...
  const isDive = id === 'the-dive';
  
  return (
    <a href={`/bar/${id}`} className="block group">
      <div className={`relative rounded-2xl overflow-hidden border-2 ${isDive ? 'border-green-500' : 'border-gray-700'} hover:border-neon-pink transition-all duration-300 transform hover:scale-[1.02] hover:shadow-2xl`}>
        {/* Venue Image/Background */}
        <div className={`h-44 md:h-56 ${gradient} relative flex items-center justify-center`}>
          <div className={`absolute inset-0 ${isDive ? 'bg-green-900 bg-opacity-20' : 'bg-black bg-opacity-30'}`} />
          ...
          {/* St. Patrick's badge for The Dive */}
          {isDive && (
            <div className="absolute top-4 left-4 bg-green-600 rounded-full px-3 py-2 flex items-center gap-1 shadow-lg">
              <span className="text-2xl">☘️</span>
              <span className="text-white font-bold text-xs">St. Paddy&apos;s</span>
            </div>
          )}
